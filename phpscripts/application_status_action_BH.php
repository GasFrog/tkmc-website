...
if ($_POST['button_type'] == 'Approve Application') {
	...
	if ($result) {
		...
		$query = 'INSERT INTO ' . $ktable . ' (FNAME, LNAME, STREET_ADDRESS, CITY, STATE, ZIP_CODE, COUNTRY, PHONE, EMAIL, LODGE, REFERRED_BY, MENTOR, DATE_APPROVED, DATE_JOINED, NOTES) VALUES ("' . $arow['FNAME'] . '", "' . $arow['LNAME'] . '", "' . $arow['STREET_ADDRESS'] . '", "' . $arow['CITY'] . '", "' . $arow['STATE'] . '", "' . $arow['ZIP_CODE'] . '", "' . $arow['COUNTRY'] . '", "' . $arow['PHONE'] . '", "' . $arow['EMAIL'] . '", "' . $arow['LODGE'] . '", "' . $arow['REFERRED_BY'] . '", "' . $arow['MENTOR'] . '", "' . date("Y-m-d") . '", "' . $arow['DATE_APPLIED'] . '", "' . $notes . '")';
		$result = mysql_query($query, $db->connection);
		if ($result) {
			$button_action = 3;
			...
			mail('reaper@templarknightsmc.com, johnny@templarknightsmc.com, hollywood@templarknightsmc.com, bruce@templarknightsmc.com, swan@templarknightsmc.com, ghost@templarknightsmc.com, tank@templarknightsmc.com, ' . $lodgeEmail, $subject , $message, $headers);
...
switch ($button_action) {
	case 1:
		header("Location: http://templarknightsmc.com/council-area/grand-council/application-status?reject");
		break;
	case 2:
		header("Location: http://templarknightsmc.com/council-area/grand-council/application-status?update&applicant_id=".$_POST['record_num']);
		break;
	case 3:
		header("Location: http://templarknightsmc.com/council-area/grand-council/application-status?approved");
		break;
}
