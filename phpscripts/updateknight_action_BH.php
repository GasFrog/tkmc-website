...
if($result) {
	if($grandCouncil) {
		header("Location: http://templarknightsmc.com/council-area/grand-council/update-knight?success&knightNum=".$recordNum);
	} else {
		header("Location: http://templarknightsmc.com/council-area/update-knight?success&knightNum=".$recordNum);
	}
} else {
	echo 'Failed to update Record for ' . $firstName . ' ' . $lastName . ' ERROR: ' . mysql_fetch_array($result) . '<br>Query: ' . $query;
}
