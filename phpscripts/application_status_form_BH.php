...
if (!isset($_GET['applicant_id'])) {
	...
	if ($result) {
		while($row = mysql_fetch_array($result)){
			if ($row['LODGE']) {
				...
				$lresult = mysql_query($lquery, $db->connection);
				if ($lresult) {
			...
			echo '<p><a href="templarknightsmc.com/council-area/grand-council/application-status?applicant_id='.$row['RECORD_NUM'].'">'.$row['FNAME'].' '.$row['LNAME'].'</a> - '.$lodgeName.' (Date Applied: '.$row['DATE_APPLIED'].')';
	...
	echo '<p><a href="http://templarknightsmc.com/council-area/grand-council/application-status">Back</a></p>';
	...
	if ($result) {
		...
		echo '<div style="display:none;"><input type="hidden" name="processing_by" value="'.$current_user->user_login.'"></div>';
		echo '<p style="text-align:center;">';
		if ($row['PROCESSING_CHECK']) {
			echo '<input type="checkbox" name="processing_check" value="1" checked />Application is being processed by '.ucfirst($row['PROCESSING_BY']).'<br>';
		} else {
			echo '<input type="checkbox" name="processing_check" value="1" />Processing Application.<br>';
		}
		echo '</p>';
		echo '<table>';
		if ($row['CRUISER_CHECK']) {
			echo '<tr><td><input type="checkbox" name="cruiser_check" value="1" checked />Motorcycle meets style and size regulations.</td>';
		} else {
			echo '<tr><td><input type="checkbox" name="cruiser_check" value="1" />Motorcycle meets style and size regulations.</td>';
		}
		if ($row['AFFILIATIONS_CHECK']) {
			echo '<td><input type="checkbox" name="affiliations_check" value="1" checked />Other club memberships and affiliations have been verified.</td></tr>';
		} else {
			echo '<td><input type="checkbox" name="affiliations_check" value="1"/>Other club memberships and affiliations have been verified.</td></tr>';
		}
		if ($row['FB_CHECK']) {
			echo '<tr><td><input type="checkbox" name="fb_check" value="1" checked/>Facebook and other social media have been verified.</td>';
		} else {
			echo '<tr><td><input type="checkbox" name="fb_check" value="1"/>Facebook and other social media have been verified.</td>';
		}
		if ($row['SPOKEO_CHECK']) {
			echo '<td><input type="checkbox" name="spokeo_check" value="1" checked/>Spokeo and other sites have been checked.</td></tr>';
		} else {
			echo '<td><input type="checkbox" name="spokeo_check" value="1" />Spokeo and other sites have been checked.</td></tr>';
		}
		if ($row['MILITARY_CHECK']) {
			echo '<tr><td><input type="checkbox" name="military_check" value="1" checked />Military and DOD records have been verified.</td>';
		} else {
			echo '<tr><td><input type="checkbox" name="military_check" value="1" />Military and DOD records have been verified.</td>';
		}
		if ($row['FELONY_CHECK']) {	
			echo '<td><input type="checkbox" name="felony_check" value="1" checked/>Felony background information has been verified.</td></tr>';
		} else {
			echo '<td><input type="checkbox" name="felony_check" value="1" />Felony background information has been verified.</td></tr>';
		}
		if ($row['DRIVING_CHECK']) {
			echo '<tr><td><input type="checkbox" name="driving_check" value="1" checked />Driving record verified.</td>';
		} else {
			echo '<tr><td><input type="checkbox" name="driving_check" value="1" />Driving record verified.</td>';
		}
		if ($row['INSURANCE_CHECK']) {	
			echo '<td><input type="checkbox" name="insurance_check" value="1" checked/>Insurance verified.</td></tr>';
		} else {
			echo '<td><input type="checkbox" name="insurance_check" value="1" />Insurance verified.</td></tr>';
		}
		if ($row['APPFEE_CHECK']) {	
			echo '<tr><td><input type="checkbox" name="appfee_check" value="1" checked/>Application Fee received.</td>';
		} else {
			echo '<tr><td><input type="checkbox" name="appfee_check" value="1" />Application Fee received.</td>';
		}
		echo '<td></td></tr>';
		...
		echo '<p>Notes:<br><textarea name="notes" cols="100" rows="5">'.$row['NOTES'].'</textarea></p>';
		echo '<p><input type="submit" name="button_type" value="Update Application">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" name="button_type" value="Approve Application">&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" name="button_type" value="Reject Application"></p>';
		echo '<hr><hr>';
		// Form information
